...
class InstanceSegmentation(pl.LightningModule):
    def __init__(self, config):
        ...
        for i in range(self.model.num_levels * self.model.num_decoders):
            if i not in self.config.general.ignore_mask_idx:
                ...
                aux_weight_dict.update(
                    {k + f"_{i}": 0. for k, v in weight_dict.items()})
    ...
    def training_step(self, batch, batch_idx):
        ...
        try:
            output = self.forward(data,
                                  point2segment=[target[i]['point2segment']
        ...
        logs = {f"train_{k}": v.detach().cpu().item()
        ...
        # Remove CUDA Cache after one training step
        ...
        with open(f"{base_path}/{file_name}.txt", "w") as fout:
            ...
            for instance_id in range(len(pred_classes)):
                ...
                if score > self.config.general.export_threshold:
                    ...
                    print(f"Generating the output data: "
    ...
    def training_epoch_end(self, outputs):
        # print(f"\n#################### Entering training_epoch_end ####################")
        train_loss = sum([out["loss"].cpu().item()
    ...
    def save_visualizations(self, target_full, full_res_coords,
        ...
        print("Entering save_visualizations...")
        ...
        if 'labels' in target_full:
            ...
            for instance_counter, (label, mask) in enumerate(zip(target_full['labels'], target_full['masks'])):
                ...
                mask_coords_min = full_res_coords[mask_tmp.astype(
                    bool), :].min(axis=0)
                mask_coords_max = full_res_coords[mask_tmp.astype(
                    bool), :].max(axis=0)
                ...
                gt_pcd_color.append(
                    self.validation_dataset.map2color(
                        [label]).repeat(gt_pcd_pos[-1].shape[0], 1)
                )
                gt_inst_pcd_color.append(instances_colors[instance_counter % len(
                    instances_colors)].unsqueeze(0).repeat(gt_pcd_pos[-1].shape[0], 1))
        ...
        for did in range(len(sorted_masks)):
            ...
            for i in reversed(range(sorted_masks[did].shape[1])):
                coords = full_res_coords[sorted_masks[did]
                                         [:, i].astype(bool), :]

                mask_coords = full_res_coords[sorted_masks[did][:, i].astype(
                    bool), :]
                mask_normals = original_normals[sorted_masks[did][:, i].astype(
                    bool), :]
                ...
                if self.config.data.test_mode != 'test':
                    ...
                    pred_sem_color.append(
                        self.test_dataset.map2color([label]).repeat(
                            mask_coords.shape[0], 1)
                    )
                ...
                # For test
                # test_color = instances_colors[i % len(instances_colors)].unsqueeze(0).repeat(
                #     mask_coords.shape[0], 1)
                # test_coords = mask_coords
                # test_color = np.asarray(test_color)
                # test_coords = np.asarray(test_coords)

                # print(f"instances_color is: {test_color}")
                # o3d_pcd = o3d.geometry.PointCloud()
                # o3d_pcd.points = o3d.utility.Vector3dVector(test_coords)
                # o3d_pcd.colors = o3d.utility.Vector3dVector(test_color / 255)
                # #o3d_pcd.normals = o3d.utility.Vector3dVector(pred_normals)

                # folder_out = f"{self.config['general']['save_dir']}/visualizations/{file_name}_ply"
                # if not os.path.exists(folder_out):
                #     print("creating...")
                #     os.mkdir(folder_out)
                # out_path = f"{folder_out}/test_inst{i}.ply"
                # o3d.io.write_point_cloud(out_path, o3d_pcd, write_ascii=True)
            ...
            if len(pred_coords) > 0:
                ...
                v.add_points("Instances (Mask3D)", pred_coords,
                             ...
                             alpha=0.8,
                             point_size=point_size)
        print("Generating visualized data...")
        
        # o3d_pcd = o3d.geometry.PointCloud()
        # o3d_pcd_ori = o3d.geometry.PointCloud()

        # np_alpha = np.ones(pred_inst_color.shape[0]) * 0.8
        # np_alpha = np_alpha.reshape((np_alpha.shape[0], 1))
        # # pred_inst_color = np.hstack([pred_inst_color, np_alpha])

        # o3d_pcd_ori.points = o3d.utility.Vector3dVector(full_res_coords)
        # o3d_pcd_ori.colors = o3d.utility.Vector3dVector(original_colors / 255)
        # o3d_pcd_ori.normals = o3d.utility.Vector3dVector(original_normals)

        # o3d_pcd.points = o3d.utility.Vector3dVector(pred_coords)
        # o3d_pcd.colors = o3d.utility.Vector3dVector(pred_inst_color / 255)
        # o3d_pcd.normals = o3d.utility.Vector3dVector(pred_normals)

        # ori_out_path = f"{self.config['general']['save_dir']}/visualizations/{file_name}_ori.ply"
        # out_path = f"{self.config['general']['save_dir']}/visualizations/{file_name}.ply"
        # o3d.io.write_point_cloud(ori_out_path, o3d_pcd_ori, write_ascii=True)
        # o3d.io.write_point_cloud(out_path, o3d_pcd, write_ascii=True)
    ...
    def eval_step(self, batch, batch_idx):
        ...
        data = ME.SparseTensor(coordinates=data.coordinates,
                               features=data.features, device=self.device)
        ...
        if self.config.general.save_visualizations:
            print("save_visualization is on, forming backbone_feature...")
            backbone_features = output['backbone_features'].F.detach(
            ).cpu().numpy()
    ...
    def get_full_res_mask(self, mask, inverse_map, point2segment_full, is_heatmap=False):
        ...
        if self.eval_on_segments and is_heatmap == False:
            mask = scatter_mean(mask, point2segment_full,
                                dim=0)  # full res segments
            mask = (mask > 0.5).float()
            # full res points
    ...
    def get_mask_and_scores(self, mask_cls, mask_pred, num_queries=100, num_classes=18, device=None):
        if device is None:
            device = self.device
        labels = torch.arange(num_classes, device=device).unsqueeze(
        ...
        if self.config.general.topk_per_image != -1:
            ...
            scores_per_query, topk_indices = mask_cls.flatten(
                0, 1).topk(num_queries, sorted=True)

        labels_per_query = labels[topk_indices]

        # topk_indices = topk_indices // num_classes
        topk_indices = torch.div(
            topk_indices, num_classes, rounding_mode='trunc')
        ...
        mask_scores_per_image = (
            heatmap * result_pred_mask).sum(0) / (result_pred_mask.sum(0) + 1e-6)
        score = scores_per_query * mask_scores_per_image
        classes = labels_per_query

        return score, result_pred_mask, classes, heatmap

    def eval_instance_step(self, output, target_low_res, target_full_res, inverse_maps, file_names, full_res_coords,
                original_colors, original_normals, raw_coords, idx, first_full_res=False, backbone_features=None):
        # print("Enter eval_instance_step!")
        # print(f"the file_names are: {file_names}")
        if self.config.data.test_mode:
            label_offset = self.test_dataset.label_offset
        else:
        ...
        for bid in range(len(prediction[self.decoder_id]['pred_masks'])):
            ...
            if not first_full_res:
                ...
                if self.model.train_on_segments:
                    masks = prediction[self.decoder_id]['pred_masks'][bid].detach(
                ...
                if self.config.general.use_dbscan:
                    ...
                    curr_coords = raw_coords[offset_coords_idx:
                                             curr_coords_idx + offset_coords_idx]
                    ...
                    for curr_query in range(masks.shape[1]):
                        ...
                        if curr_coords[curr_masks].shape[0] > 0:
                            ...
                            new_mask[curr_masks] = torch.from_numpy(
                                clusters) + 1
                    ...
                    scores, masks, classes, heatmap = self.get_mask_and_scores(
                        prediction[self.decoder_id]['pred_logits'][bid].detach(
                        ).cpu(),
                        masks,
                    ...
                    if not (sort_scores_values[instance_id] < self.config.general.scores_threshold):
                        ...
                        if not sorted_masks[:, instance_id].sum() == 0.0:
                            overlap_ids = set(np.nonzero(
                                norm_overlaps[instance_id, :] > self.config.general.iou_threshold)[0])
        ...
        for bid in range(len(prediction[self.decoder_id]['pred_masks'])):
            if self.config.data.test_mode:
                all_pred_classes[bid] = self.test_dataset._remap_model_output(
            ...
            if self.config.data.test_mode != "test" and len(target_full_res) != 0:
                ...
                # PREDICTION BOX
                bbox_data = []
                for query_id in range(all_pred_masks[bid].shape[1]):
                    obj_coords = full_res_coords[bid][all_pred_masks[bid][:, query_id].astype(
                        bool), :]

                    if obj_coords.shape[0] > 0:
                        obj_center = obj_coords.mean(axis=0)
                        obj_axis_length = obj_coords.max(
                            axis=0) - obj_coords.min(axis=0)

                        bbox = np.concatenate((obj_center, obj_axis_length))

                        # print(f"query_id.item() is: {all_pred_classes[bid][query_id].item()}")
                ...
                for obj_id in range(target_full_res[bid]['masks'].shape[0]):
                    ...
                    obj_coords = full_res_coords[bid][target_full_res[bid]['masks'][obj_id, :].cpu(
            ...
            if self.config.general.save_visualizations:
                if 'cond_inner' in self.test_dataset.data[idx[bid]]:
                    ...
                    self.save_visualizations(target_full_res[bid],
                                             full_res_coords[bid][self.test_dataset.data[idx[bid]]
                                                                  ['cond_inner']],
                                             [self.preds[file_names[bid]]
                                                 ['pred_masks']],
                                             [self.preds[file_names[bid]]
                                                 ['pred_classes']],
                                             file_names[bid],
                                             original_colors[bid][self.test_dataset.data[idx[bid]]
                                                                  ['cond_inner']],
                                             original_normals[bid][self.test_dataset.data[idx[bid]]
                                                                   ['cond_inner']],
                                             [self.preds[file_names[bid]]
                                                 ['pred_scores']],
                                             sorted_heatmaps=[
                                                 all_heatmaps[bid][self.test_dataset.data[idx[bid]]['cond_inner']]],
                                             query_pos=all_query_pos[bid][self.test_dataset.data[idx[bid]]['cond_inner']] if len(
                                                 all_query_pos) > 0 else None,
                    ...
                    self.save_visualizations(target_full_res[bid],
                                             ...
                                             sorted_heatmaps=[
                                                 all_heatmaps[bid]],
                                             query_pos=all_query_pos[bid] if len(
                                                 all_query_pos) > 0 else None,
                                             backbone_features=backbone_features,
                                             point_size=self.config.general.visualization_point_size)
    ...
    def eval_instance_epoch_end(self):
        # print("Enter eval_instance_epoch_end...")
        log_prefix = f"val"
        ap_results = {}

        head_results, tail_results, common_results = [], [], []

        box_ap_50 = eval_det(self.bbox_preds, self.bbox_gt,
                             ovthresh=0.5, use_07_metric=False)
        box_ap_25 = eval_det(self.bbox_preds, self.bbox_gt,
                             ovthresh=0.25, use_07_metric=False)
        mean_box_ap_25 = sum(
            [v for k, v in box_ap_25[-1].items()]) / len(box_ap_25[-1].keys())
        mean_box_ap_50 = sum(
            [v for k, v in box_ap_50[-1].items()]) / len(box_ap_50[-1].keys())
        ...
        try:
            ...
            mprec, mrec = evaluate(
                new_preds, gt_data_path, pred_path, dataset_yaml=self.validation_dataset.dataset_yaml)
            ...
            mean_ap_50 = statistics.mean(
                [item for key, item in ap_results.items() if key.endswith("val_ap_50")])
            mean_ap_25 = statistics.mean(
                [item for key, item in ap_results.items() if key.endswith("val_ap_25")])
            ...
            ap_results = {key: 0. if math.isnan(
                score) else score for key, score in ap_results.items()}

        except (IndexError, OSError) as e:
            print(f"NO SCORES!!! Error is {e}")
