...
# minimum region size for evaluation [verts] (100 for s3dis, scannet)
opt['min_region_sizes'] = np.array([100])
opt['distance_threshes'] = np.array([float('inf')])  # distance thresholds [m]
opt['distance_confs'] = np.array([-float('inf')])  # distance confidences
...
def evaluate_matches(matches):
    ...
    ap = np.zeros((len(dist_threshes), len(
        CLASS_LABELS), len(overlaps)), float)
        ...
        for oi, overlap_th in enumerate(overlaps):
            ...
            for li, label_name in enumerate(CLASS_LABELS):
                ...
                for m in matches:
                    ...
                    gt_instances = [gt for gt in gt_instances if gt['instance_id'] >= 1000 and gt['vert_count'] >=
                                    min_region_size and gt['med_dist'] <= distance_thresh and gt['dist_conf'] >= distance_conf]
                    if gt_instances:
                        has_gt = True
                    if pred_instances:
                        has_pred = True
...
def make_pred_info(pred: dict):
    ...
    pred_info = {}
    assert (pred['pred_classes'].shape[0] ==
            pred['pred_scores'].shape[0] == pred['pred_masks'].shape[1])
...
def assign_instances_for_scan(pred: dict, gt_file: str):
    ...
    for uuid in pred_info:
        ...
        for (gt_num, gt_inst) in enumerate(gt2pred[label_name]):
            intersection = np.count_nonzero(np.logical_and(
                gt_ids == gt_inst['instance_id'], pred_mask))
            if intersection > 0:
...
def print_results(avgs):
    ...
    data.append([" Item Types ", "    AP   ", "  AP_50% ", "  AP_25% "])
    for (li, label_name) in enumerate(CLASS_LABELS):
        ap_avg = avgs["classes"][label_name]["ap"]
        ap_50o = avgs["classes"][label_name]["ap50%"]
        ap_25o = avgs["classes"][label_name]["ap25%"]
        data.append([label_name, ap_avg, ap_50o, ap_25o])
    all_ap_avg = avgs["all_ap"]
    all_ap_50o = avgs["all_ap_50%"]
    all_ap_25o = avgs["all_ap_25%"]
    data.append(["Average", all_ap_avg, all_ap_50o, all_ap_25o])
    table = tabulate(data, headers='firstrow', tablefmt='psql', colalign=col_align,
                     floatfmt=(None, '.3f', '.3f', '.3f'))
    horizontal_line = table.split('\n')[0]
...
def evaluate(preds: dict, gt_path: str, output_file: str, dataset_yaml: str):
    global CLASS_LABELS
    ...
    print(f"\nthe dataset for evaluation is: {dataset}")

    CLASS_LABELS = list(class_map.keys())
    VALID_CLASS_IDS = np.array([i for i in range(1, len(CLASS_LABELS) + 1)])
    ID_TO_LABEL = {}
    LABEL_TO_ID = {}
    for i in range(len(VALID_CLASS_IDS)):
    ...
    # if dataset == "abbpcd_bin":
    #     CLASS_LABELS = ['dustpan', 'envelope', 'ipad_cover',
    #                     'pencil_box', 'piab_box', 'white_box', 'bin']
    #     VALID_CLASS_IDS = np.array([1, 2, 3, 4, 5, 6, 7])
    #     ID_TO_LABEL = {}
    #     LABEL_TO_ID = {}
    #     for i in range(len(VALID_CLASS_IDS)):
    #         LABEL_TO_ID[CLASS_LABELS[i]] = VALID_CLASS_IDS[i]
    ...
    for i, (k, v) in enumerate(preds.items()):
        ...
        print(f"the gt_file should be: {gt_file}")
        ...
        for inst_id in reversed(range(v['pred_masks'].shape[1])):
            point_ids = np.argwhere(
                v['pred_masks'][:, inst_id] == 1.)[:, 0]
        ...
        for ig, g in enumerate(un):  # each object in prediction
            if g == -1:
                continue
            tmp = (pred_ins == g)
            sem_seg_i = int(stats.mode(pred_sem[tmp], keepdims=True)[0])
        ...
        for ig, g in enumerate(un):
            tmp = (gt_ins == g)
            sem_seg_i = int(stats.mode(gt_sem[tmp], keepdims=True)[0])
        ...
        sys.stdout.write("\rscans processed: {}\n".format(i+1))
        sys.stdout.flush()
    ...
    for i_sem in range(NUM_CLASSES):
        ...
        # Ignore the warning by dividing NaN
