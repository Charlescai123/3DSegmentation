...
class VoxelizeCollate:
    def __init__(
        ...
        assert task in ["instance_segmentation",
                        "semantic_segmentation"], "task not known"
        self.task = task
        ...
        if (
            ...
            for i in range(0, len(batch), self.scenes):
                ...
                for j in range(min(len(batch[i:]), self.scenes)):
                    ...
                    max_instance_id, max_segment_id = batch[i + j][2].max(axis=0)[
                        1:]
...
def voxelize(batch, ignore_label, voxel_size, probing, mode, task,
    ...
    for sample in batch:
        ...
        voxelization_dict.update({"coordinates": torch.from_numpy(
            coords).to("cpu").contiguous(), "features": sample[1]})
    ...
    if mode == "test":
        ...
        if "labels" in input_dict:
            for i in range(len(input_dict["labels"])):
                ...
                _, ret_index, ret_inv = np.unique(
                    input_dict["labels"][i][:, -1], return_index=True, return_inverse=True)
    ...
    if "train" not in mode:
        ...
        return (
            NoGpu(coordinates, features, original_labels,
                  inverse_maps, full_res_coords), target,
            [sample[3] for sample in batch]
        )
...
def get_instance_masks(list_labels, task, list_segments=None, ignore_class_threshold=100,
    ...
    for batch_id in range(len(list_labels)):
        ...
        for instance_id in instance_ids:
            ...
            # instance == -1 ???
            tmp = list_labels[batch_id][list_labels[batch_id]
                                        [:, 1] == instance_id]
            label_id = tmp[0, 0]
            ...
            if list_segments:
                segment_mask = torch.zeros(
                    list_segments[batch_id].shape[0]).bool()
                segment_mask[list_labels[batch_id][list_labels[batch_id]
                                                   [:, 1] == instance_id][:, 2].unique()] = True
