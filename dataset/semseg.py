...
import numpy
import torch
from dataset.random_cuboid import RandomCuboid
...
class SemanticSegmentationDataset(Dataset):
    ...
    def __init__(
        self,
        dataset_name="abbpcd",
        dataset_yaml="dataset/configuration/dataset_yaml/abbpcd.yaml",
        data_dir: Optional[Union[str, Tuple[str]]] = "data/processed/abbpcd",
        ...
        assert task in ["instance_segmentation",
                        "semantic_segmentation"], "unknown task"
        ...
        for database_path in self.data_dir:
            ...
            if not (database_path / f"{self.mode}_database.yaml").exists():
                print(
                    f"Please generate {database_path}/{self.mode}_database.yaml first!!!")
                exit()

            self._data.extend(self._load_yaml(
                database_path / f"{self.mode}_database.yaml"))

        if data_percent < 1.0:
            self._data = sample(self._data, int(
                len(self._data) * data_percent))
    ...
    def splitPointCloud(self, cloud, size=50.0, stride=50, inner_core=-1):
        if inner_core == -1:
            ...
            cells = [(x * stride, y * stride)
                     for x in range(width) for y in range(depth)]
            blocks = []
            for (x, y) in cells:
            ...
            conds_inner = []
            for (x, y) in cells:
                xcond_outer = (cloud[:, 0] <= x + inner_core / 2. + size /
                               2) & (cloud[:, 0] >= x + inner_core / 2. - size / 2)
                ycond_outer = (cloud[:, 1] <= y + inner_core / 2. + size /
                               2) & (cloud[:, 1] >= y + inner_core / 2. - size / 2)

                cond_outer = xcond_outer & ycond_outer
                block_outer = cloud[cond_outer, :]

                xcond_inner = (
                    block_outer[:, 0] <= x + inner_core) & (block_outer[:, 0] >= x)
                ycond_inner = (
                    block_outer[:, 1] <= y + inner_core) & (block_outer[:, 1] >= y)
    ...
    def __getitem__(self, idx: int):
        ...
        if "train" in self.mode or self.is_tta:
            ...
            try:
                coordinates += np.random.uniform(coordinates.min(0),
                                                 coordinates.max(0)) / 2
            ...
            color = np.squeeze(self.image_augmentations(
                image=pseudo_image)["image"])
            ...
            if self.add_unlabeled_pc:
                if random() < 0.8:
                    ...
                    pseudo_image = unlabeled_color.astype(
                        np.uint8)[np.newaxis, :, :]
    ...
    def _remap_from_zero(self, labels):
        labels[~np.isin(labels, list(self.label_info.keys()))
               ] = self.ignore_label
...
def flip_in_center(coordinates):
    ...
    if second_crop.size > 1:
        ...
        minimum[2] = 0
        minimum[0] = 0
        coordinates[second_crop] = aug(
            points=coordinates[second_crop])["points"]
    ...
    if fourth_crop.size > 1:
        ...
        minimum[2] = 0
        coordinates[fourth_crop] = aug(
            points=coordinates[fourth_crop])["points"]
...
def random_points(
    ...
    noisy_coordinates = np.array(
        list(
            product(
                np.linspace(min_boundary[0],
                            max_boundary[0], noisy_coordinates),
                np.linspace(min_boundary[1],
                            max_boundary[1], noisy_coordinates),
                np.linspace(min_boundary[2],
                            max_boundary[2], noisy_coordinates),
            )
        )
    )
    noisy_coordinates += np.random.uniform(
