...
class BasePreprocessingTest:
    ...
    def preprocess(self):
        ...
        for mode in self.modes:
            ...
            self.save_database(database, mode)  # save Area_X_database.yaml
        self.fix_bugs_in_labels()
        #self.joint_database()       # concatenate for training (train + validate dataset)
        self.compute_color_mean_std(
            train_database_path=(self.save_dir / "test_database.yaml")
        )
        shutil.copy(self.save_dir / "test_database.yaml",self.save_dir / "train_database.yaml")
    ...
    def extract_instance_from_file(self, sample_from_database):
        ...
        for instance_id in np.unique(labels[:, 1]):
            ...
            instance_classes = np.unique(
                instance_points[:, 9]).astype(int).tolist()

            hash_string = str(
                sample_from_database["filepath"]) + str(instance_id)
            hash_string = md5(hash_string.encode("utf-8")).hexdigest()
            instance_filepath = self.save_dir / \
                "instances" / f"{hash_string}.npy"
            instance = {
    ...
    def compute_color_mean_std(
        self, train_database_path: str = "./data/processed/test_database.yaml",
    ):
        pass
    ...
    def joint_database(self, train_modes=["train", "validation"]):
        ...
        for mode in train_modes:
            joint_db.extend(self._load_yaml(
                self.save_dir / (mode + "_database.yaml")))
        self._save_yaml(
    ...
    def _save_yaml(cls, path, file):
        with open(path, "w") as f:
            yaml.safe_dump(file, f, default_style=None,
