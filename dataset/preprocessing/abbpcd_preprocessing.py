...
class ABBPCDPreprocessing(BasePreprocessing):
    def __init__(
            self,
            data_dir: str = "./data/raw/abbpcd_box",
            save_dir: str = "./data/processed/abbpcd_box",
            modes: tuple = ("train", "validation", "test"),
            dataset_yaml: str = "./dataset/configuration/dataset_yaml/abbpcd_box.yaml",
            n_jobs: int = -1,
    ):
    ...
    def process_file(self, filepath, mode):
        ...
        for instance in [f for f in os.scandir(self.data_dir / mode / scene_name / "Annotations")
            ...
            instance_class = self.class_map[instance.name.rsplit("_", maxsplit=1)[
                0]]
            ...
            cluster_points.append(instance_points)
            instance_counter += 1

        points = np.vstack(cluster_points)
